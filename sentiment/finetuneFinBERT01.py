...
train_df = pd.read_csv(
    './data/train.csv', usecols=['content', 'relativity'])
val_df = pd.read_csv(
    './data/dev.csv', usecols=['content', 'relativity'])
test_df = pd.read_csv(
    './data/test.csv', usecols=['content', 'relativity'])
...
# -balance
train_df1 = train_df[train_df.relativity == 1].sample(
    n=8000, replace=False, random_state=None, axis=None)
train_df0 = train_df[train_df.relativity != 1]
train_df = pd.concat([train_df0, train_df1])


val_df1 = val_df[val_df.relativity == 1].sample(
    n=3000, replace=False, random_state=None, axis=None)
val_df0 = val_df[val_df.relativity != 1]
val_df = pd.concat([val_df0, val_df1])

test_df1 = test_df[test_df.relativity == 1].sample(
    n=3000, replace=False, random_state=None, axis=None)
test_df0 = test_df[test_df.relativity != 1]
...
torch.save(train_loader, './dataloader/train_loader-more-balance64-32-32-256.pt')
torch.save(val_loader, './dataloader/val_loader-more-balance64-32-32-256.pt')
torch.save(test_loader, './dataloader/test_loader-more-balance64-32-32-256.pt')
...
#---------train model---------#
OUT_PATH = "./results_model/more-balance64-32-32-256/"
...
for epoch in range(epochs):
    ...
    train_loader = torch.load(
        './dataloader/train_loader-more-balance64-32-32-256.pt')
    ...
    val_acc = 0.0
    val_f1 = 0.0
    val_loader = torch.load('./dataloader/val_loader-more-balance64-32-32-256.pt')
...
start = time.time()
test_loader = torch.load('./dataloader/test_loader-more-balance64-32-32-256.pt')
